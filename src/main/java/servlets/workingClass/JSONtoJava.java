	...
	private Object handleJsonObject(JsonObject jsonObject, Class<?> parameterClass) throws Throwable {
		...
		if (parameterClass.isInstance(Map.class)) {
			if (parameterClass == Map.class) {
				...
				Class<?> genericClassForKey = ((ParameterizedType) genericTypes[0]).getActualTypeArguments()[0]
						.getClass();
				Class<?> genericClassForValue = ((ParameterizedType) genericTypes[1]).getActualTypeArguments()[0]
						.getClass();
				for (JsonString key : keyset) {
					returnmap.put(getJavaObject(key, genericClassForKey),
							getJavaObject(entries.get(key), genericClassForValue));
				}
